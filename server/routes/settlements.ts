...
async function verifyNimiqTx(
  ...
  try {
    const res = await fetch(NIMIQ_RPC_URL, {
      ...
      body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'getTransactionByHash', params: { hash: txHash } }),
...
router.get('/:id/settlements', requireAuth, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const rows = await sql<{
      id: string; group_id: string; from_addr: string; to_addr: string;
      amount: string; currency: string; tx_hash: string | null;
      verified_at: Date | null; created_at: Date;
    }[]>`
    ...
    res.json(rows.map((r) => ({
      ...
      currency: r.currency,
      settledAt: r.verified_at ?? r.created_at,
    })));
  } catch (err) {
    console.error(err);
...
router.post('/:id/settlements', requireAuth, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const body = req.body as {
      ...
      currency: string; txHash: string; settledAt?: string;
    };
    ...
    if (normalizeAddr(body.fromId) !== normalizeAddr(address)) {
      res.status(403).json({ error: 'fromId must match authenticated user' }); return;
    }

    // Idempotent par tx_hash
    const existing = await sql`
      SELECT 1 FROM payments WHERE group_id = ${groupId} AND tx_hash = ${body.txHash} LIMIT 1
    `;
    if (existing.length > 0) { res.status(200).end(); return; }

    // Vérification on-chain via Nimiq RPC
    const verified = await verifyNimiqTx(body.txHash, body.fromId, body.toId, body.amount, groupId);
    if (verified === 'invalid') {
      res.status(422).json({ error: 'transaction does not match settlement data' }); return;
    }

    // 'valid' → verified_at = now ; 'not_found' → verified_at = null (RPC indisponible)
